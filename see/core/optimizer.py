...
class Optimizer:
    def __init__(self, config, model):
        ...
        elif config.NAME == "Adam-ExposureHead":
            # Differential LR: exposure_head at HEAD_LR_MULT × backbone LR.
            # Called before nn.DataParallel, so model.exposure_head is directly accessible.
            head_params    = list(model.exposure_head.parameters())
            head_ids       = {id(p) for p in head_params}
            backbone_params = [p for p in model.parameters() if id(p) not in head_ids]
            head_lr = config.LR * config.HEAD_LR_MULT
            info(f"Adam-ExposureHead: backbone_lr={config.LR}  head_lr={head_lr}")
            self.optimizer = Adam([
                {"params": backbone_params, "lr": config.LR},
                {"params": head_params,     "lr": head_lr},
            ])
        elif config.NAME == "Adam-ToneHead":
            # Differential LR: tone_head at HEAD_LR_MULT × backbone LR.
            # Called before nn.DataParallel, so model.tone_head is directly accessible.
            head_params     = list(model.tone_head.parameters())
            head_ids        = {id(p) for p in head_params}
            backbone_params = [p for p in model.parameters() if id(p) not in head_ids]
            head_lr = config.LR * config.HEAD_LR_MULT
            info(f"Adam-ToneHead: backbone_lr={config.LR}  head_lr={head_lr}")
            self.optimizer = Adam([
                {"params": backbone_params, "lr": config.LR},
                {"params": head_params,     "lr": head_lr},
            ])
        elif config.NAME == "Adam-GainHead":
            # Train only requires_grad=True params (backbone is frozen).
            # model.gain_head and model.nl_cond_mlp are trainable; backbone is not.
            trainable = [p for p in model.parameters() if p.requires_grad]
            info(f"Adam-GainHead: trainable_params={sum(p.numel() for p in trainable)}  lr={config.LR}")
            self.optimizer = Adam(trainable, lr=config.LR)
        else:
